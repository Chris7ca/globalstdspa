<template>
	...
	<ul class="pagination uk-pagination" role="navigation" v-if="total > perPage">
		...
		<li v-for="(page, i) in visualPages" :key="i">
			<a role="button" :class="{ 'uk-text-bold uk-background-primary' : (currentPage  == page) }" @click="getPage(page)">
				<span :class="{ 'uk-light' : (currentPage  == page) }">{{ page }}</span>
			</a>
		</li>

		<li v-if="pages >= 2">
			<a role="button" @click="getPage(lastPage)" uk-icon="icon: chevron-double-right" uk-tooltip="Last page"></a>
		</li>
